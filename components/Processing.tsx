...
const groupTexts = [
  "단체톡에서 대화를 읽고 있습니다...",
  "누가 말했는지 참가자를 구분하고 있습니다...",
  "나와 각 사람의 티키타카를 비교하고 있습니다...",
  "AI 친구가 눈에 띈 기류를 정리하고 있습니다...",
];
...
export default function Processing() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const images = uploadStorage.read() ?? [];
  ...
  if (error) {
    ...
    return <div className="analysis-error">
      ...
      <AppButton fullWidth onClick={() => {
        ...
        router.replace("/analyze/upload");
      }}>{missingApiKey ? "설정 후 다시 시도하기" : suggestedMode ? (suggestedMode === "group" ? "단체톡 모드로 바꾸기" : "1:1 모드로 바꾸기") : "캡처 다시 올리기"}</AppButton>
    </div>;
  }
  ...
  return <>
    ...
    <div className="processing-actions">
      <AppButton variant="secondary" fullWidth className="cancel-analysis" onClick={cancel} disabled={cancelling}>{cancelling ? "취소 중..." : "분석 취소하기"}</AppButton>
